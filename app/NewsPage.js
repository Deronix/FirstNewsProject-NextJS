...
const NewsDataProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    setNewsData(savedData ? JSON.parse(savedData) : persistentNewsData);
    
    const handleStorage = (e) => {
      if (e.key === 'newsData') {
        setNewsData(JSON.parse(e.newValue));
      }
    };
...
// Persistent news data
const persistentNewsData = [// Persistent news data with localStorage integration
    {
      id: "haber1",
    ...
    },
    {
      ...
      not1: ["2024 Q4: Avrupa Medikal Cihaz Regülasyonu (MDR) sertifikasyonu alındı."],
    ...
    {
      id: "haber5",
      ana_baslik: "Tokamak Tabanlı Kompakt Füzyon Reaktörü",
      kisa_tanitim:
      ...
      not1: ["2025 Q1: IAEA Füzyon Güvenlik Protokolü Tier-4 sertifikasyonu alındı"],
    ...
    },
    {
      ...
      not1: ["2025 Q2: ICAO Annex 17 Uzay Güvenlik Standardı sertifikasyonu tamamlandı"],
      not2: ["2027 Q4: Ekvator hattında ilk operasyonel tesisin inşasına başlanacak"],
...
const NewsPageComponent = ({ newsData }) => {
...
export default function NewsPage() {
  return (
    <NewsDataProvider>
      {(newsData) => <NewsPageComponent newsData={newsData} />}
    </NewsDataProvider>
  );
}